...
{:else}
	...
	<div class="flex items-center justify-between h-8">
		...
		>
			<span
				class="absolute top-[2px] w-[14px] h-[14px] rounded-full transition-all duration-150
					{config['web.enabled'] !== false ? 'left-[17px] bg-white dark:bg-black' : 'left-[2px] bg-white dark:bg-gray-500'}"
			></span>
		</button>
	</div>
	<p class="text-[11px] text-gray-400 dark:text-gray-600 mt-0.5">
	...
	{#if config['web.enabled'] !== false}
		...
		<div class="flex items-center justify-between h-8 mt-3">
			<span class="text-[13px] text-gray-700 dark:text-gray-300">{$t('admin.webSearchProvider')}</span>
			<select
				...
				onchange={(e) => updateConfig('web.search_provider', (e.target as HTMLSelectElement).value)}
				disabled={saving}
			>
		...
		{:else if provider === 'exa'}
			<div class="mt-3">
				<label class="block text-[13px] text-gray-700 dark:text-gray-300 mb-1">{$t('admin.webExaKey')}</label>
				<input
					type="password"
		...
		{:else if provider === 'tavily'}
			<div class="mt-3">
				<label class="block text-[13px] text-gray-700 dark:text-gray-300 mb-1">{$t('admin.webTavilyKey')}</label>
				<input
					type="password"
					class="w-full text-[13px] bg-gray-50 dark:bg-white/4 border border-gray-200 dark:border-white/8 rounded-lg px-2.5 py-1.5 outline-none text-gray-700 dark:text-gray-300 placeholder:text-gray-400 dark:placeholder:text-gray-600"
					placeholder="tvly-..."
		...
		{:else if provider === 'brave'}
			<div class="mt-3">
				<label class="block text-[13px] text-gray-700 dark:text-gray-300 mb-1">{$t('admin.webBraveKey')}</label>
				<input
					type="password"
					class="w-full text-[13px] bg-gray-50 dark:bg-white/4 border border-gray-200 dark:border-white/8 rounded-lg px-2.5 py-1.5 outline-none text-gray-700 dark:text-gray-300 placeholder:text-gray-400 dark:placeholder:text-gray-600"
					placeholder="BSA..."
		...
		{:else if provider === 'duckduckgo'}
			<p class="text-[11px] text-gray-400 dark:text-gray-600 mt-2">{$t('admin.webDuckDuckGoNote')}</p>
		{/if}
	{/if}
